...
namespace FastReportWebCore.MVC.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index(int? reportIndex = 0)
        {
            ...
            model.WebReport.Report.Load(System.IO.Path.Combine(
                System.IO.Path.GetDirectoryName(this.GetType().Assembly.Location), "Reports", reportToLoad + ".frx"));

            var dataSet = new DataSet();
            dataSet.ReadXml(System.IO.Path.Combine(
                System.IO.Path.GetDirectoryName(this.GetType().Assembly.Location), "Reports", "nwind.xml"));
